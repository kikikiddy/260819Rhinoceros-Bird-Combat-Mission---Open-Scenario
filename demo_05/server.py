"""T12 交互式演示服务 · 版本 demo_05（仅依赖标准库 + 项目既有依赖，无需额外安装）。

启动方式：
    python demo_05/warmup.py                 # 【录屏前建议先跑】预生成 7 个示例合同的审阅缓存
    python demo_05/server.py                 # 默认 http://127.0.0.1:8005
    python demo_05/server.py --port 9000    # 指定端口
    HY3_API_KEY=xxx python demo_05/server.py
...
说明：
    ...
    - 磁盘缓存（demo_05/.cache）：同一份合同文本只真实调用一次，重复演示秒出、零额外 token；
      录屏前用 warmup.py 预热，演示时点击任意示例均即时返回。

本版本（demo_05）相对 demo_04 的改动（按用户"每版都保存"约定，新建版本、保留 demo_04 作历史）：
    ⓪ 新增审阅结果磁盘缓存 + warmup.py 预热脚本：解决演示时每点一次都要等模型生成的问题，
       预热后点击示例即时出结果，且不会重复消耗 token。

本版本（demo_05）相对 demo_03 的改动（按用户"每版都保存"约定，新建版本、保留 demo_03 作历史）：
...
PORT = int(os.environ.get("DEMO_PORT", "8005"))
MAX_TOKENS = int(os.environ.get("DEMO_MAX_TOKENS", "1800"))
USE_CACHE = os.environ.get("DEMO_USE_CACHE", "1") not in ("0", "false", "False", "")

HTML_PATH = os.path.join(ROOT, "demo_05", "index.html")
CACHE_DIR = os.path.join(ROOT, "demo_05", ".cache")


def _cache_path(text: str) -> str:
    import hashlib

    return os.path.join(CACHE_DIR, hashlib.md5(_norm(text).encode("utf-8")).hexdigest() + ".json")


def cache_get(text: str) -> dict | None:
    """命中则返回缓存的审阅结果 dict（与 ReviewResult.to_dict() 同构）。"""
    if not USE_CACHE:
        return None
    try:
        p = _cache_path(text)
...
def cache_put(text: str, result: dict) -> None:
    if not USE_CACHE:
        return
    try:
        os.makedirs(CACHE_DIR, exist_ok=True)
        open(_cache_path(text), "w", encoding="utf-8").write(
    ...
    def do_POST(self):
        ...
        try:
            ...
            if cached is not None:
                result = type("R", (), {"to_dict": lambda self, d=cached: d, "source": "live"})()
                from_cache = True
            else:
                client = build_client()
...
def main() -> None:
    ...
    print(f"[demo_05] 服务已启动： http://{args.host}:{args.port}/")
    print(f"[demo_05] 合同库样本数： {len(CONTRACT_DB)} | 评测模型： {model}")
    print(f"[demo_05] 密钥： {key_state} | max_tokens： {MAX_TOKENS}")
    print("[demo_05] 按 Ctrl+C 停止。")
    try:
        srv.serve_forever()
